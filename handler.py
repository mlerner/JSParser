...
class ViewParseAjaxHandler(BaseHandler):
    ...
    def getFormattedTimestamp(self):
        ...
        formatted = "{}_{}_{}_{}-{}".format(d.month, d.day, d.year, d.hour,
                                            d.minute)
        return formatted
        ...
        if parsedLinks:
            ...
            for link in parsedLinks:
                ...
                if self.isLongLine(link["line"]):
                    highlightedLine = '...{}<span class="highlight">{}</span>{}...'.format(
                        escape(link["line"][startPos - 100:startPos]),
                        link["link"], escape(link["line"][endPos:100]))
                else:
                    highlightedLine = '{}<span class="highlight">{}</span>{}'.format(
                        escape(link["line"][:startPos]), link["link"],
                        escape(link["line"][endPos:]))
                # generate the link HTML
                html = html + '<div class="link">{}: {}</div>'.format(
